...
import { saveGraph, loadGraph } from '../graph/storage';
import { registerProject, listProjects, unregisterProject } from '../graph/registry';
import { generateClaudeMd } from '../claude-md/generator';
import { startMcpServer } from '../mcp/server';
import { startWebServer } from '../web/server';
import { setupProject } from '../setup';

program.name('cgraph').description('Code intelligence graph for AI agents').version('1.0.0');
...
program
  ...
  .action(async (projectPath: string, options: { claudeMd: boolean; force: boolean }) => {
    const abs = path.resolve(projectPath);

    const existing = loadGraph(abs);
    ...
    const result = await parseProject(abs);
    const graph = buildGraph(abs, result);
...
program
  ...
  .action(async (projectPath: string, options: { mcpPort: string; webPort: string }) => {
    const abs = path.resolve(projectPath);
    ...
    await Promise.all([startMcpServer(graph, mcpPort), startWebServer(mcpPort, webPort)]);
...
program
  ...
  .action((projectPath: string, options: { mcpPort: string }) => {
    const abs = path.resolve(projectPath);
    console.log(`\n⚙️  Setting up Claude Code integration for: ${abs}\n`);
    setupProject(abs, parseInt(options.mcpPort, 10));
  });
